# RealTimeDataMiningInAction
# Summary

[前言]

* [1. 应用场景及典型问题](README.md)
   * [1.1 流式数据挖掘的应用场景](application/README.md)
   * [1.2 典型问题举例](application/README.md)
      * [1.2.1 在线欺诈检测问题](README.md)
      * [1.2.2 游戏挂机实时干预问题](README.md)

* [2. 流式数据简介](README.md)
   * [2.1 流式数据的基本概念](README.md)
   * [2.2 流式数据的特点](README.md)
   * [2.3 流式数据的底层架构（以Flink为例）](README.md)
   * [2.4 CACE理论](README.md)

* [3. 策略及工程挑战](README.md)
   * [3.1 在线实时决策的策略挑战](README.md)
   * [3.2 在线实时决策的工程挑战](README.md)

* [4. 机器学习方法与实践](MachineLearningMethodAndPractice/MLDescription.md)
   * [4.1 机器学习的基本概念及分类](MachineLearningMethodAndPractice/MLConcepts.md)
   * [4.2 机器学习的一般方法及建模步骤](MachineLearningMethodAndPractice/Steps.md)
   * [4.3 机器学习方法用于在线实时决策实践](application/README.md)
      * [4.3.1 如何选取标记样本](application/README.md)
      * [4.3.2 确定合适的建模目标](application/README.md)
      * [4.3.3 建立baseline](application/README.md)
      * [4.3.4 特征工程](application/README.md)
      * [4.3.5 模型选择](application/README.md)
   * [4.4 方法小结](application/README.md)
      * [4.4.1 NFL理论](MachineLearningMethodAndPractice/Methods/NFL.md)
      * [4.4.2 MVP理论](application/README.md)

* [5. 效果评估方案选取](application/README.md)
   * [5.1 评估指标选取](application/README.md)
   * [5.2 实验流量选取](application/README.md)
      * [5.2.1 离线评估](application/README.md)
      * [5.2.2 在线评估](application/README.md)
   * [5.3 评估方法和技巧小结](application/README.md)

* [6. 总结与展望](application/README.md)
